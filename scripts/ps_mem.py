    ...
    def open(self, *args):
        try:
            ...
            if (
                ...
                or val.errno == errno.EPERM
                or val.errno == errno.EACCES
            ):
                raise LookupError
            raise
...
def getCmdName(pid, split_args, discriminate_by_pid, exe_only=False):
    ...
    try:
        ...
        val = sys.exc_info()[1]
        if (
            val.errno == errno.ENOENT  # either kernel thread or process gone
            or val.errno == errno.EPERM
            or val.errno == errno.EACCES
        ):
            raise LookupError
...
def print_memory_usage(
    sorted_cmds, shareds, count, total, swaps, total_swap, show_swap
):
    ...
    if have_swap_pss and show_swap:  # kernel will have_pss
        sys.stdout.write(
            "%s\n%s%9s%s%9s\n%s\n"
            % ("-" * 45, " " * 24, human(total), " " * 3, human(total_swap), "=" * 45)
        )
    elif have_pss:
        sys.stdout.write(
...
def main():
    ...
    split_args, pids_to_show, watch, only_total, discriminate_by_pid, show_swap = (
        parse_options()
    )
